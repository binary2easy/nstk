...
%       contourWidth = 1;
%       connectivity = 4;
%       [starting_points, ending_points] = CCMS_Contour(data, l(1), connectivity, displayflag, contourColor, contourWidth);
%         ps = starting_points;
%         pe = ending_points;
%         contourcolor = [1 0 0];
%         contourWidth = 1;
%         h = RenderContour([], ps, pe, contourcolor, contourWidth);
%         axis square;  axis manual;
%         axis([1 g.shape(1) 1 g.shape(2)]);
%         drawnow
     case 'surf'
   ...
   case 3
    switch(displayType)
     ...
     case 'slice'
      ...
      avgy = mean(g.vs{2});
      avgz = mean(g.vs{3});
      slice(g.vs{1}, g.vs{2}, g.vs{3}, data, avgx, avgy, avgz);
      
     case 'contourslice'
      avgx = mean(g.vs{1});
      avgy = mean(g.vs{2});
      avgz = mean(g.vs{3});
      h = contourslice(g.xs{1}, g.xs{2}, g.xs{3}, ls, [ avgx ], [ avgy ], [ avgz ], l);
