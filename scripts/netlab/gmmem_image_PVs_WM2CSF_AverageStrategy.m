function [mix, options, errlog] = gmmem_image_PVs_WM2CSF_AverageStrategy(mix, x, options)
...
for n = 1:niters
    ...
    if ( mod(n, mix.PVStep) == 0 )
        ...
        filename = ['segResults_Average/segResult_gmmPVs_results' num2str(n) '.hdr' ];
        ...
        if ( mix.ncentres == 4 )
            [LabeledSeg, mix] = LabelPVs_Seg_gmm_4classes_AverageStrategy(mix, results, ...
                                                csflabel, wmlabel, cortexlabel, pvlabel,...
                                                nonbrainlabel);
                                            
%             [LabeledSeg, mix] = LabelPVs_Seg_gmm_4classes_MEX(mix, results, ...
%                                                 csflabel, wmlabel, cortexlabel, pvlabel,...
%                                                 nonbrainlabel);
...
%                                         nonbrainlabel);
            ...
            [LabeledSeg, mix] = LabelPVs_Seg_gmm_5classes_MEX(mix, results, ...
                                        csflabel, wmlabel, cortexlabel, pvlabel,...
                                        nonbrainlabel);

        end
        
        filename = ['segResults_Average/segResult_gmmPVs_LabeledSeg_' num2str(n) '.hdr' ];
