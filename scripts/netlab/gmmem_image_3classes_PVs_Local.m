function [mix, options, errlog] = gmmem_image_3classes_PVs_Local(mix, x, options, partsmask, globalSeg, globalClassNum)
...
for n = 1:niters
    ...
    if ( mod(n, mix.PVStep) == 0 )
        ...
        for pp = 1:ndata
            ...
            if ( label(1) == 1 ) % gm
                results(mix.indexes(pp, 1), mix.indexes(pp, 2), mix.indexes(pp, 3)) = cortexlabel;
            end
            
            if ( globalClassNum == 5 )           
                if ( label(1) == 2 ) % wm1
                    results(mix.indexes(pp, 1), mix.indexes(pp, 2), mix.indexes(pp, 3)) = wmlabel(1);
                end
                
                if ( label(1) == 3 ) % wm2
                    results(mix.indexes(pp, 1), mix.indexes(pp, 2), mix.indexes(pp, 3)) = wmlabel(2);
                end
            end
        ...
        if ( mix.ncentres == 4 )
            [LabeledSeg, mix] = LabelPVs_Seg_gmm_4classes_Local_MEX(mix, newglobalSeg, ...
                                                csflabel, wmlabel, cortexlabel, pvlabel,...
                                                nonbrainlabel, partsmask, globalSeg);
        else          
            [LabeledSeg, mix] = LabelPVs_Seg_gmm_5classes_Local_MEX(mix, newglobalSeg, ...
                                        csflabel, wmlabel, cortexlabel, pvlabel,...
                                        nonbrainlabel, partsmask, globalSeg);
        end
