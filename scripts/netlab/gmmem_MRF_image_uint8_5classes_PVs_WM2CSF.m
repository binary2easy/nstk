function [mix, options, errlog] = gmmem_MRF_image_uint8_5classes_PVs_WM2CSF(mix, x, options)
...
for n = 1:niters
    ...
    %     mix.centres
    %     mix.covars
        mix = Prior2Classification_uint8_quick(mix);
    %     mix = Prior2Classification_uint8(mix);
    %     row = mix.indexes(:,1);
    %     col = mix.indexes(:,2);
    %     plane = mix.indexes(:,3);
    ...
    %     mix = Classification2Prior_uint8(mix);
        mix = Classification2Prior_uint8_quick(mix);
    end
    % removal PVs
