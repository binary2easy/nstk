function [mix, options, errlog] = gmmem_MRF_image_uint8_4classes_PVs_WM2CSF(mix, x, options)
...
% Hui modified this to add the MRF prior step to deal with partial volume voxels
...
for n = 1:niters
  ...
  if ( mod(n, 2) ~= 0 )
    ...
    %   mix = Prior2Classification_uint8(mix);
    %     mix.centres
    %     mix.covars
    %     mix = Prior2Classification_uint8_4classes(mix);
        mix = Prior2Classification_uint8_4classes_quick(mix)
    %     row = mix.indexes(:,1);
    %     col = mix.indexes(:,2);
    %     plane = mix.indexes(:,3);
        [G, H] = ems_getMRFparams(mix.classification, mix.lkp, mix.sampleInd, 1);
        G
        H
        %[G, H] = ems_getMRFparams_uint8_noSampleInd(mix.classification, mix.lkp, row, col, plane, 1);
        % compute MRF prior
        mix.classification(:) = ems_getMRFprior(mix.classification, uint8(mix.playing), mix.atlas, mix.lkp, G, H);
    %     mix = Classification2Prior_uint8(mix);
    %     mix = Classification2Prior_uint8_4classes(mix);
        mix = Classification2Prior_uint8_4classes_quick(mix)
  end
  ...
  if ( mod(n, 2) == 0 )
        ...
        if ( mix.ncentres == 4 )
            [LabeledSeg, mix] = LabelPVs_Seg_gmm_4classes_atlas_Global_MEX(mix, results, ...
                                                csflabel, wmlabel, cortexlabel, pvlabel,...
                                                nonbrainlabel);
        else
    %             [LabeledSeg, mix] = LabelPVs_Seg_gmm_5classes(mix, results, ...
    %                                         csflabel, wmlabel, cortexlabel, pvlabel,...
    %                                         nonbrainlabel);            

            [LabeledSeg, mix] = LabelPVs_Seg_gmm_5classes_atlas_Global_MEX(mix, results, ...
                                        csflabel, wmlabel, cortexlabel, pvlabel,...
                                        nonbrainlabel);
        end
        ...
        mix = LabelPVs_WM2CSF(mix, results, LabeledSeg, csflabel, cortexlabel, wmlabel, pvlabel, mix.VolumeThres);
