function [mix, options, errlog] = gmmem_image_PVs_WM2CSF_NonPrior(mix, x, options)
...
for n = 1:niters
    ...
    if ( mod(n, mix.PVStep) == 0 )
        ...
        filename = ['segResults_NonPrior/segResult_gmmPVs_results' num2str(n) '.hdr' ];
        ...
        if ( mix.ncentres == 4 )
%             [LabeledSeg, mix] = LabelPVs_Seg_gmm_4classes(mix, results, ...
...
%                                         nonbrainlabel);
        ...
        filename = ['segResults_NonPrior/segResult_gmmPVs_LabeledSeg_' num2str(n) '.hdr' ];
